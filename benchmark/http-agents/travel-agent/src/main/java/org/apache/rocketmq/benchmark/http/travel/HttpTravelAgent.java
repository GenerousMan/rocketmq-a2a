 ...
 */
package org.apache.rocketmq.benchmark.http.travel;
...
/**
 * HTTP直连版行程Agent - 使用HTTP协议直接通信
 * 
 * <p>模拟传统的HTTP直连架构，所有Agent通信通过HTTP REST API完成。
 * LLM调用被Mock为固定延迟+固定响应。
 */
public class HttpTravelAgent {
    
    private static final Logger log = LoggerFactory.getLogger(HttpTravelAgent.class);
    ...
    public HttpTravelAgent(int port) {
        this.port = port;
    }
    
    public static void main(String[] args) throws IOException {
        int port = args.length > 0 ? Integer.parseInt(args[0]) : DEFAULT_PORT;
        HttpTravelAgent agent = new HttpTravelAgent(port);
        agent.start();
        
        log.info("HTTP Travel Agent started on port {}", port);
    ...
    public void stop() {
        if (server != null) {
            server.stop(0);
            log.info("HTTP Travel Agent stopped");
        }
    }
